    ...
    describe 'sqlEngine.setDb', ->
      ...
      describe 'sqlEngine.execute', ->
        ...
        it "should return undefined if query syntax is correct, but there is no result for", ->
          engine.setDb(extraSmallDb)
          expect(engine.execute('* from jsdlfkjsdlf')).toBeUndefined()
        ...
        it "should be able to work with the WHERE guery (equal operator)", ->
          ...
          it "should work if the some side in where conditions is string in quotes", ->
            ...
            expect(engine.execute("ctor.name from actor where actor.name = 'Bradley Cooper'")).toEqual
              Sql_result: [
                { 'actor.name': 'Bradley Cooper'},
              ]

        it "should return the result if the condition in WHERE part is apply between the columns", ->
          engine.setDb(mockDb)
        ...
        it "should return 'Error state' if in such WHERE conditions ('<', '>', '>=', '<=', '<>' operator) used the stings", ->
          engine.setDb(mockDb)
...
#        xit "should work with the cross-join query", ->
#          engine.setDb(extraSmallDb)
#          expect(engine.execute('jsdlfkjsdlf')).toBe('Error state')