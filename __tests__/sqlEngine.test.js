...
describe('sqlEngine', function() {
  ...
  describe('setDb', function() {
    ...
    it('should return data DB which set up', function() {
      expect(engine.setDb(mockDB)).toEqual(mockDB);
    });

    it('should called with mock DB', function() {
  ...
  describe('getDbStuff', function() {
    ...
    it('should add the name of the table to each column in', function() {
  ...
  describe('execute', function() {
    ...
    it('should called with string', function() {
    ...
    it('should return object if a query is correct', function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('* from one')).toBeInstanceOf(Object);
    });

    it('should return undefined if query syntax is correct, but there is no result for', function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('* from jsdlfkjsdlf')).toBe('Error state');
    });

    it("should return 'Error state' if query is incorrect", function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('jsdlfkjsdlf')).toBe('Error state');
    });

    it('should return result for selecting all field from table', function() {
      engine.setDb(extraSmallDB);
    ...
    it('should return result for selecting the specific field from the table', function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('one.id from one')).toEqual({
        Sql_result: [{ 'one.id': 1 }, { 'one.id': 2 }],
      });
    });

    it('should return result for selecting the specific fields from the table', function() {
      engine.setDb(mockDB);
    ...
    it("should return 'Error state' if some columns not presented in the table", function() {
      engine.setDb(mockDB);
    ...
    it('should be able to work with the WHERE query (equal operator)', function() {
      engine.setDb(mockDB);

      expect(engine.execute('actor.id, actor.name from actor where actor.id = 2')).toEqual({
        Sql_result: [{ 'actor.id': 2, 'actor.name': 'Bradley Cooper' }],
      });
      ...
      expect(engine.execute('actor.name from actor where actor.id = 2')).toEqual({
        Sql_result: [{ 'actor.name': 'Bradley Cooper' }],
      });
    });

    it('should work if the some side in WHERE conditions is string in quotes', function() {
      engine.setDb(mockDB);
    ...
    it('should return the result if the condition in WHERE part applied between the columns', function() {
      engine.setDb(mockDB);
    ...
    it('should return all result if the condition in the WHERE part is just a boolean', function() {
      engine.setDb(extraSmallDB);
    ...
    it('should be error work with the WHERE condition is incorrect or don`t match anything', function() {
      engine.setDb(mockDB);
    ...
    it("should be able to work with such WHERE conditions ('<', '>', '>=', '<=', '<>'  operator)", function() {
      engine.setDb(mockDB);
    ...
    it("should return 'Error state' if in WHERE conditions within ('<', '>', '>=', '<=', '<>') operators used the stings", function() {
      engine.setDb(mockDB);
    ...
    it("should be able to work with multiple WHERE conditions which may separated only by 'or' or 'and' but not together in one query", function() {
      engine.setDb(mockDB);
    ...
    it("should be able to work with multiple WHERE conditions which may separated only by 'or' or 'and' but not together in one query", function() {
      ...
      expect(
        ...
        Sql_result: [{ 'actor.name': 'Bradley Cooper', 'actor.id': 2 }],
      });
    ...
    it('should work with the cross-join query', function() {
      engine.setDb(extraSmallDB);
    ...
    it("should be 'Error state' if in Cross-join query presented duplicate tables", function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('* from one, two, one')).toBe('Error state');
    });

    it('should be able to work with JOIN query *on Table1.prop = Table2.prop*', function() {
      engine.setDb(extraSmallDB);
    ...
    it('should be able to work with JOIN query correctly if the the left side *on Table1.prop = Table2.prop* is changes', function() {
      engine.setDb(extraSmallDB);
    ...
    it('should be able to work with JOIN query and certain columns', function() {
      engine.setDb(extraSmallDB);
    ...
    it("should be 'Error state' in JOIN query 'tableFrom' equal 'tableON' *select 'some stuff' from tableFrom join tableON ...*", function() {
      engine.setDb(extraSmallDB);
      expect(engine.execute('* from one join one on one.one_id = one.id')).toBe('Error state');
    });

    it("should be 'Error state' in JOIN query some columns not presented in tables *select 'some stuff' from tableFrom join tableON tableFrom.asdjajksd = tableFrom.asd ...*", function() {
      engine.setDb(extraSmallDB);
    ...
    it('should be able to compute multiple joins in JOIN query', function() {
      engine.setDb(extraSmallDB);
    ...
    it("should be 'Error state' in multiple JOIN query if some table presented in query more than one time", function() {
      engine.setDb(extraSmallDB);
    ...
    it('should work all together', function() {
      ...
      expect(
        ...
        Sql_result: [{ 'actor.id': 1, 'director.name': 'Joe Carnahan', 'some_stuff.director_id': 5 }],
      });
    });
  });
});
